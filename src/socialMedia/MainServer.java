...
public class MainServer extends RemoteObject implements ServerInterface{
	...
	public void start() {
		...
		String server, tcpPortStr, udpPort, multiCast, multiCastPort, registryHost, registryPortStr, sleepTime, serverName, serverClassName;
		...
		Thread t = new Thread(new CalculateSystem(users, Integer.parseInt(sleepTime), objectMapper, usersJson, multiCastPortInt, multiCast));
		t.start();
			...
			for(Post postComment : posts) {
				if(postComment.getPostId() == idPost3) {
					...
					if(!postComment.checkComment(name)) {
						// Se e' la prima volta che commenta si conta
						postComment.addNewPeopleCommenting();
